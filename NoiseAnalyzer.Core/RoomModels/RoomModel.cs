...
namespace NoiseAnalyzer.Core.RoomModels
{
    public class RoomModel
    {
        ...
        {
            return ToString(Defaults.PrintDigitsPrecision);
        }
        ...
        public string ToString(int precision)
        {
            ...
            builder.AppendLine($"\n{RoomName}:");
            builder.AppendLine($"Room internal source Lw  {RoomSourceLw.Format(precision)} dB");
            builder.AppendLine($"Room internal source Lp {RoomSourceLp.Format(precision)} dB");
            builder.AppendLine($"Room resulting Lp {RoomOutLp.Format(precision)} dB");
            builder.AppendLine($"Total room equivalent absorbption area {RoomAbsEquivArea.Format(precision)} m2");
            builder.AppendLine($"Distance from terminal {MinDistR.RoundUp(precision)} m");
            builder.AppendLine($"Total room sound power Lp {RoomSumOutLp.RoundUp(precision)} dB");
